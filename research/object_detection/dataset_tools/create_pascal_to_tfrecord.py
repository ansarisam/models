...
def create_tf(examples_list, annotations_dir, label_map_dict, dataset_type):
   ...
   for idx, example in enumerate(examples_list):
       ...
       if idx % 100 == 0:
           ...
           writer = tf.python_io.TFRecordWriter(FLAGS.output_path + "/"+dataset_type+"/tf_"+dataset_type+"_" + str(j) + ".record")
           j = j + 1

       path = os.path.join(annotations_dir, os.path.basename(example).replace(".jpg", '.xml').replace(".png", '.xml'))
